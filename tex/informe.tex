...
\begin{ocl}{Toda Preferencia de Coche, el coche del viaje (si tiene viaje), es uno de los preferidos}
  ...
  inv: self.solicitudes $\rightarrow$ forAll( x | x.viaje.isNotEmpty() implies self.taxis $\rightarrow$ include(x.viaje.taxi))
\end{ocl}
...
\begin{ocl}{Toda Preferencia de Taxista, el Taxista del viaje (si tiene viaje), es uno de los preferidos.}
  ...
  inv: self.solicitudes $\rightarrow$ forAll( x | x.viaje.isNotEmpty() implies self.taxistas $\rightarrow$ include(x.viaje.taxista))
\end{ocl}
...
\begin{ocl}{Toda Preferencia de Calificación del taxista, el taxista del viaje (si tiene viaje), tiene calificación mayor o igual a la elegida.}
  ...
  inv: self.solicitudes $\rightarrow$ forAll( s | s.viaje.isNotEmpty() implies s.viaje.taxista.viajes $\rightarrow$ select(v.calificación | v.isKindOf(Viaje de Usuario Registrado) and v.calificación.isNotEmpty()) $\rightarrow$ collect(c | c.puntaje) $\rightarrow$ mean() >= self.calificación )
\end{ocl}
% 
