...
\section{Diagrama de contexto}
  ...
  \begin{figure}[H]
    \includegraphics[height=12cm,angle=90]{diagramas/contexto.png}
  \end{figure}
...
\begin{usecasesteps}
  ...
  \ucstep{El sistema corrobora que al menos un taxista haya aceptado y le informa a la aplicación los 10 más adecuados que hayan aceptado (o a todos si hay menos que 10)}{\ucalt{No hay candidatos disponibles que satisfagan las preferencias del cliente. El sistema informa a la aplicación que se precisan condiciones menos restrictivas. Ir a paso 1.}}
...
\begin{usecase}
  \ucname{Generando viaje periódico}
  \ucpre{Un usuario logueado completó el formulario de solicitud de viaje periódico, en la aplicación.}
  \ucpost{El sistema registra una nueva solicitud de viaje periódico.}
  \ucactor{Aplicación}
\end{usecase}
\begin{usecasesteps}
  \ucstep{La aplicación envía al sistema una solicitud de viaje incluyendo las direcciones de orígen y destino y la periodicidad del viaje.}{}
  \ucstep{El sistema verifica que los datos recibidos sean válidos.}{\ucalt{Los datos son inválidos. El sistema informa a la aplicación que no se pudo completar la acción. Fin caso de uso.}}
  \ucstep{El sistema guarda un recordatorio que generará automáticamente un viaje, en el momento indicado.}{}
  \ucstep{El sistema confirma al cliente que su solicitud de viaje periódico se realizó con éxito.}{}
  \ucstep{Fin caso de uso.}{}
\end{usecasesteps}

\begin{usecase}
  \ucname{Consultando posición del taxista}
  \ucpre{Un usuario logueado que ya solicitó un viaje, quiere saber la posición del taxista.}
  \ucpost{El sistema le envía la posición del taxista asociado a su viaje, al cliente.}
  \ucactor{Aplicación}
\end{usecase}
\begin{usecasesteps}
  \ucstep{La aplicación le pregunta al sistema acerca de la posición del taxista asociado al viaje del usuario.}{}
  \ucstep{El sistema calcula la hora estimada de llegada en base a la posición guardada del taxista.}{\ucalt{Si no tiene los datos de la posición actual del taxista, envía ``No se conoce la posición del taxista''.}}
  \ucstep{El sistema envía la hora estimada de llegada}{}
  \ucstep{Fin caso de uso.}{}
\end{usecasesteps}
...
\begin{usecase} 
  ...
  \ucpre{El Cliente esta logueado}
  \ucpost{Hay un nueva calificación guardada en el sistema.}
  \ucactor{Aplicación}
\end{usecase}
\begin{usecasesteps}
  \ucstep{La aplicación manda la calificación del cliente.}{}
  \ucstep{El sistema verifica que el viaje que se esta calificando corresponda al cliente en cuesti\'on}{\ucalt{Le env\'ia un error a la aplicación diciendo que el viaje calificado no corresponde con el cliente.}}
  \ucstep{Fin caso de uso.}{}
\end{usecasesteps}
...
\begin{usecase} 
  ...
  \ucpre{True}
  \ucpost{La aplicación env\'ia la solicitud al taxista}
  \ucactor{Aplicación M\'ovil}
\end{usecase}
\begin{usecasesteps}
  \ucstep{El sistema le manda la solicitud a la aplicación para cierto taxista.}{}
  \ucstep{La Apliación le manda la solicitud al taxista.}{}
  \ucstep{Fin caso de uso.}{}
\end{usecasesteps}
...
\begin{usecase}
  ...
  \ucpre{True}
  \ucpost{Se le aviso al taxista la cancelación del viaje}
  \ucactor{Aplicación M\'ovil}
\end{usecase}
\begin{usecasesteps}
  \ucstep{El sistema le pide a la aplicación m\'ovil que avise al taxista de la cancelación.}{}
  \ucstep{La aplicación le avisa al taxista.}{}
  \ucstep{Fin caso de uso.}{}
\end{usecasesteps}
...
\begin{usecase}
  ...
  \ucpre{E taxista respondi\'o}
  \ucpost{El sistema sabe la respuesta del taxista}
  \ucactor{Aplicación M\'ovil}
\end{usecase}
\begin{usecasesteps}
  \ucstep{La aplicación le informa al sistema la respuesta del taxista.}{}
  \ucstep{Fin caso de uso.}{}
\end{usecasesteps}
...
\begin{usecase} 
  ...
  \ucpre{True}
  \ucpost{Se le aviso al taxista la Confirmación del viaje}
  \ucactor{Aplicación M\'ovil}
\end{usecase}
\begin{usecasesteps}
  \ucstep{El sistema le pide a la aplicación m\'ovil que confirme al taxista el viaje.}{}
  \ucstep{La aplicación le confirma al taxista el viaje.}{}
  \ucstep{Fin caso de uso.}{}
\end{usecasesteps}
...
\end{usecase}
\begin{usecasesteps}
  \ucstep{El Operador informa acerca de la modificación de estado al sistema}{}
...
\begin{usecasesteps}
  ...
  \ucstep{El directivo indica que desea visualizar los indicadores.}{}
  \ucstep{El sistema muestra los indicadores.}{}
  \ucstep{Fin caso de uso.}{}
\end{usecasesteps}
...
\begin{usecase}
  \ucname{Agregando Taxista}
  \ucpre{El taxista a agregar no existe en el sistema}
  \ucpost{El taxista fue agregado al sistema}
  \ucactor{Directivos}
\end{usecase}
\begin{usecasesteps}
  \ucstep{El directivo se autentica.}{}
  \ucstep{El sistema valida los datos.}{\ucalt{Si el sistema no puede validar los datos y es el tercer intento, se bloquea. Si el sistema no puede validar los datos y no es el tercer intento, ir a paso 1.}}
  \ucstep{El directivo indica que desea agregar un nuevo taxista al sistema.}{}
  \ucstep{El sistema muestra el formulario de incorporación de un taxista.}{}
  \ucstep{El directivo completa los datos del nuevo taxista y confirma la operación.}{}
  \ucstep{El sistema sistema verifica que los datos sean válidos.}{\ucalt{Si los datos son inválidos, el sistema indica que hubo un error. Ir a paso 5.}}
  \ucstep{Los datos ingresados son válidos; el sistema incorpora el nuevo taxista en sus registros.}{}
  \ucstep{Fin caso de uso.}{}
\end{usecasesteps}
...
\begin{usecase}
  \ucname{Modificando Taxista}
  \ucpre{El taxista a modificar existe en el sistema}
  \ucpost{Los datos del taxista fueron modificados dentro del sistema}
  \ucactor{Directivos}
\end{usecase}
\begin{usecasesteps}
  \ucstep{El directivo se autentica.}{}
  \ucstep{El sistema valida los datos.}{\ucalt{Si el sistema no puede validar los datos y es el tercer intento, éste se bloquea. Si el sistema no puede validar los datos y no es el tercer intento, ir a paso 1.}}
  \ucstep{El directivo indica que desea modificar los datos de un taxista.}{}
  \ucstep{El sistema muestra un panel de búsqueda para los registros de taxistas.}{}
  \ucstep{El directivo localiza el taxista que desea modificar y confirma que desea realizar una modificación.}{}
  \ucstep{El sistema muestra los datos del taxista elegido junto a un formulario de edición.}{}
  \ucstep{El directivo realiza los cambios necesarios y confirma la operación.}{}
  \ucstep{El sistema sistema verifica que los datos sean válidos.}{\ucalt{Si los datos son inválidos, el sistema indica que hubo un error. Ir a paso 7.}}
  \ucstep{Los datos ingresados son válidos; el sistema guarda los cambios realizados en los datos del taxista.}{}
  \ucstep{Fin caso de uso.}{}
\end{usecasesteps}
...
\begin{usecase}
  \ucname{Eliminando Taxista}
  \ucpre{El taxista a eliminar existe en el sistema}
  \ucpost{El taxista es eliminado de los registros del sistema}
  \ucactor{Directivos}
\end{usecase}
\begin{usecasesteps}
  \ucstep{El directivo se autentica.}{}
  \ucstep{El sistema valida los datos.}{\ucalt{Si el sistema no puede validar los datos y es el tercer intento, éste se bloquea. Si el sistema no puede validar los datos y no es el tercer intento, ir a paso 1.}}
  \ucstep{El directivo indica que desea eliminar un taxista.}{}
  \ucstep{El sistema muestra un panel de búsqueda para los registros de taxistas.}{}
  \ucstep{El directivo localiza el taxista que desea eliminar e indica que desea realizar la operación.}{}
  \ucstep{El sistema pide confirmación.}{}
  \ucstep{El directivo confirma que desea realizar la operación.}{\ucalt{El directivo cancela la operación. Ir al paso 4.}}
  \ucstep{El sistema elimina efectivamente al taxista de sus registros.}{}
  \ucstep{Fin caso de uso.}{}
\end{usecasesteps}
...
En esta sección, vamos a utilizar FSM para detallar el comportamiento de los usuarios, el sistema y los taxistas al momento de realizar una solicitud y generar un viaje asociado a la misma.

La FSM que describirá este comportamiento ser\'a la generada por la composici\'on en paralelo de $n$ m\'aquinas $Cliente_i$, $n$ m\'aquinas $Sistema_i$ y $m$ m\'aquinas $Taxista_j$.
...
Y por último, $Taxista_j$ representa a uno de los taxistas de la flota.
...
\begin{itemize}
	...
	\item Usamos $Sacar\ x\ de\ a$ para referirnos a sacar al elemento $x$ del array $a$. Esto se puede desglosar en muchas transiciones no determinísticas, donde la primera tenga condición [a[0] == x] y acci\'on {a[0]=0}, la segunda sea [a[1] == x] y acci\'on {a[1]=0} y as\'i sucesivamente todas la opciones.
...
\begin{itemize}
 ...
 \item El cliente elige uno, y los dem\'as dejan de estar bloqueados, pudiendo recibir nuevas solicitudes.
