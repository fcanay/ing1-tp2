...
\begin{usecasesteps}
  ...
  \ucstep{El sistema valida los datos.}{\ucalt{Si el sistema no puede validar los datos, ir a paso 4.}} %TODO en algunos casos ponemos FIn caso de uso y otros ir a 4 (a es el fin). Si ponemos ir a 4 no estamos diciendo q temrina en curso normal?
  \ucstep{El sistema guarda los nuevos datos, enviados por el GPS.}{}
  \ucstep{Fin caso de uso.}{}
\end{usecasesteps}
...
\begin{usecase}
  \ucname{Informando Pedido Telefónico} %TODO cambiaria el nombre, es poco descriptivo, pondria algo asi como Creando solicitud telefonica
...
\begin{usecasesteps}
  ...
  \ucstep{El sistema crea la solicitud de viaje.}{} %TODO habria que describir mas el proceso de asignar taxista
  \ucstep{Fin caso de uso.}{}
\end{usecasesteps}
...
\begin{usecase}
  \ucname{Modificando Estado} %TODO poco descriptivo, habria q aclarar q es de parte del taxista
  \ucpre{El viaje existe, y el viaje asociado a la solicitud comenzó y no terminó}
  \ucpost{El sistema reconoce la modificacón de estado}
  \ucactor{Operador telefónico/radial}
\end{usecase}
\begin{usecasesteps}
  \ucstep{El Operador informa acerca de la modificacón de estado al sistema (ejemplo: se le pinchó la goma al auto)}{}
  \ucstep{El sistema valida los datos.}{\ucalt{Si el sistema no puede validar los datos, ir a paso 1.}}
  \ucstep{El sistema hace lo que puede para que el viaje pueda terminar (ejemplo: mandar otro taxi).}{}
  \ucstep{Fin caso de uso.}{}
\end{usecasesteps}
...
\begin{usecasesteps}
  ...
  \ucstep{El sistema valida los datos.}{\ucalt{Si el sistema no puede validar los datos y es el tercer intento, no se puede consultar indicadores por 5 minutos. Si el sistema no puede validar los datos y no es el tercer intento, ir a paso 1.}}
...
\begin{description} %TODO Actualizar esto con el nuevo diagrama!!!
  ...
  \item[Solicitud De Viaje Periódica:] Es una solicitud de viaje que se realiza entre las mismas direcciones origen y destino de manera periódica. %TODO medio chota esta descripcion, si quieren mejorenla
  ...
  \item[Preferencia De Coche:] Es un caso particular de Preferencia. En este caso, se refiere a que el usuario puede pedir viajar en algún modelo de coche particular.
...
\begin{ocl}{Si un viaje tiene una calificación, su Solicitud es de tipo Web/Movil}
  context Viaje
...
\begin{itemize}
	...
	\item Usamos $x\ in\ a$ para ver si el elemento $x$ pertenece al array $a$.Como el tamaño de $a$ es finito, $x\ in\ a$ se desglosa en $x\ ==\ a[0]\ ||\ ... \ x\ ==\ a[tam]$, siendo $tam$ el tamaño de $a$. Por lo que termina siendo finito.
	\item Usamos $Sacar\ x\ de\ a$ para referirnos a sacar al elemento $x$ del array $a$. Esto implica recorrer el array $a$ (el cual tiene tamaño finito) y por cada $a[i]\ ==\ x$, setear $a[i]\ ==\ 0$. Como las IDs de los taxistas son mayores o iguales que 1, efectivamente borramos dicho elemento del array.
