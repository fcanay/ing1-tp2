...
\begin{usecase}
  ...
  \ucpost{El GPS informa acerca de la posición del taxista}
  \ucactor{GPS}
\end{usecase}
\begin{usecasesteps}
...
\begin{ocl}{Cada viaje de tipo registrado el mismo usuario que su solicitud}
  context Viaje de Usuario Registrado
  inv: self.solicitud.isKindOf(Solicitud de Viaje Web/Móvil) implies
...
\begin{ocl}{Cada viaje de tipo no registrado el mismo usuario que su solicitud}
  context Viaje de Usuario No Registrado
  inv: self.solicitud.isKindOf(Solicitud de Viaje Telefónica) implies 
       self.solicitud.usuario == self.usuario
\end{ocl}
...
\begin{ocl}{Toda Preferencia de Calificación del taxista, el taxista del viaje (si tiene viaje), tiene calificación mayor o igual a la elegida.}
  ...
  inv: self.solicitudes -> forAll( s | s.viaje.isNotEmpty() implies 
       s.viaje.taxista.viajes -> select(v.calificación | v.isKindOf(Viaje de Usuario Registrado) 
       and v.calificación.isNotEmpty()) -> collect(c | c.puntaje) -> mean() >= self.calificación )
\end{ocl}
