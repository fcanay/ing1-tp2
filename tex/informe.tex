...
\begin{usecase}
  ...
  \ucpre{El taxista asociado al GPS está en el taxi}
  \ucpost{El sistema está informad acerca de la posición dada por el GPS}
  \ucactor{GPS}
\end{usecase}
\begin{usecasesteps}
  \ucstep{El GPS informa los datos del taxista al sistema.}{}
  \ucstep{El sistema valida los datos.}{\ucalt{Si el sistema no puede validar los datos, ir a paso 4.}}
...
\begin{usecase}
  \ucname{Informando Pedido Telefónico}
...
\begin{usecasesteps}
  ...
  \ucstep{El sistema crea la solicitud de viaje.}{}
  \ucstep{Fin caso de uso.}{}
\end{usecasesteps}
...
A continuación realizaremos aclaraciones sobre algunas de las clases y sociaciones presentadas previamente:
...
\begin{description}
  ...
  \item[Usuario No Registrado:] Cualquier persona que se comunique vía telefónica, sin importar si tiene un usuario registrado en el sistemao no.

  \item[Taxista:] Un empleado de la empresa que maneja taxis.

  \item[Taxi:] Un automovil de la empresa que cumple con todas las regulaciones para poder usado como taxi.

  \item[Solicitud De Viaje:] Un pedido de un cliente para realizar un viaje entre dos direcciones.

  \item[Solicitud De Viaje Web/movil:] Es un caso particular de una Solicitud de Viaje, realizada a traves de las plataformas móvil o web y con un conjunto de preferencias.

  \item[Solicitud De Viaje Telefónica:] Es un caso particular de una Solicitud de Viaje, realizada por teléfono.

  \item[Viaje:] Es la realización de una solicitud, asignandole un taxista y un taxi para la concre .
...
\begin{ocl}{Para todo viaje, el taxista maneja o manejó el taxi}
  context Viaje
  inv: (self.taxista.esta manejando == self.taxi) or (self.taxista.manejó -> includes(self.taxi)) 
\end{ocl}

\begin{ocl}{Para toda solicitud, si está en estado "esperando asignación``, no tiene viaje asignado}
  context Solicitud de Viaje
  inv: (self.estado == esperando asignación) implies (self.viaje.isEmpty()) 
\end{ocl}

\begin{ocl}{Para toda solicitud, si está en estado "en viaje" o "finalizada", tiene viaje asignado}
  context Solicitud de Viaje
  inv: (self.estado == en viaje or self.estado == finalizada) implies (self.viaje.isNotEmpty()) 
\end{ocl}

\begin{ocl}{Un taxi esta "en cochera" si y solo si, nadie lo esta manejando}
  context Taxi
  inv: (self.estado == en cochera) implies (self.manejado por.isEmpty()) and (self.manejado por.isEmpty()) implies (self.estado == en cochera) 
\end{ocl}
 
\begin{ocl}{Todo viaje que tiene una calificación, su Solicitud es de tipo Web/Movil}
  context Viaje
  inv: (self.calificación.isNotEmpty) implies (self.solicitud.isKindOf(Solicitud de Viaje Web/Movil)) 
\end{ocl}
...
\begin{ocl}{Toda Preferencia de Calificación del taxista, el taxista del viaje (si tiene viaje), tiene calificación mayor o igual a la elegida.}
  ...
  inv: self.solicitudes -> forAll( s | s.viaje.isNotEmpty() implies 
       s.viaje.taxista.viajes -> select(v.calificación | v.calificación.isNotEmpty()) -> collect(c | c.puntaje) -> mean() >= self.calificación )
\end{ocl}
